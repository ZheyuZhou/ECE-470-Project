...
returnCode, UR3_connection_handle = sim.simxGetObjectHandle(
    clientID, 'UR3_connection', sim.simx_opmode_oneshot_wait)
if returnCode != sim.simx_return_ok:
...
# np.random.rand(6,1)*2*np.pi-np.pi

theta_yellow, V = findIK(T_2in0, S, M, np.random.rand(6,1)*2*np.pi-np.pi, 1000, 0.115, 0.05)
...
# THETA_YELLOW, TF = mr.IKinSpace(S, M, Tyellow, np.random.rand(6,1)*2*np.pi-np.pi, 0.001, 0.001)
# print(THETA_YELLOW, "theta")
print("theta computation finished")
#############################################################
################## Move UR3 to Pick Yellow ##################
#############################################################
...
sim.simxSetJointPosition(clientID, UR3_joint4_handle, theta_yellow[3] + np.pi/3, sim.simx_opmode_streaming)
time.sleep(0.5)
